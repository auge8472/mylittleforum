...
{if $page_browse.browse_array[x]==$page_browse.page}<li><span class="current">{$page_browse.browse_array[x]}</span></li>{elseif $page_browse.browse_array[x]==0}<li><span>…</span></li>{else}<li><a href="index.php?mode={$mode}{if $list_spam}&amp;list_spam=1{/if}{if $action}&amp;action={$action}{/if}{if $search_encoded}&amp;search={$search_encoded}{/if}{if $method && $method!='fulltext'}&amp;method={$method}{/if}{if $id}&amp;id={$id}{/if}{if $page_browse.browse_array[x]>1}&amp;page={$page_browse.browse_array[x]}{/if}{if $p_category && $p_category>0}&amp;p_category={$p_category}{/if}{if $order}&amp;order={$order}{/if}{if $descasc}&amp;descasc={$descasc}{/if}">{$page_browse.browse_array[x]}</a></li>{/if}
{/section}
