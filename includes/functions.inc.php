...
function quote_reply($string) {
	global $settings;
	if (!empty($string))
		return preg_replace("/^/m", $settings['quote_symbol']." ", $string);
	else
		return '';
}
...
function get_edit_authorization($id, $posting_user_id, $edit_key, $time, $locked) {
	...
	if ($settings['edit_min_time_period'] != 0 && (TIMESTAMP - $settings['edit_min_time_period'] * 60) < $time)
		$edit_min_time_period_expired = false;
	else $edit_min_time_period_expired = true;

	if ($settings['edit_max_time_period'] == 0 || (TIMESTAMP - $settings['edit_max_time_period']*60) < $time)
...
function is_ip_banned($ip, $banned_ips) {
	foreach ($banned_ips as $banned_ip) {
		...
		if (strpos($banned_ip,'*') !== false) {
			...
			// $banned_ip contains "/" => CIDR notation (the regular expression is only used if $banned_ip contains "/")
